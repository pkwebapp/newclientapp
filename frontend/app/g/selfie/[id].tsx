...
export default function PublicSelfieScreen() {
  ...
  // Restore the visitor session token (needed on hard refresh / deep link).
  useEffect(() => {
    (async () => {
  ...
  if (!ready || consentGiven === null) {
    return (
      <View style={styles.center} testID="public-selfie-loading">
  ...
  if (!consentGiven) {
    return (
      <View style={styles.container} testID="public-consent-screen">
        <GlassHeader title="Before we begin" onBack={goBackToGallery} topInset={insets.top} />
        <ScrollView contentContainerStyle={[styles.consentBody, { paddingBottom: insets.bottom + spacing.xl }]}>
          ...
          <View style={{ marginTop: spacing["2xl"] }}>
            ...
            <Pressable testID="decline-consent-btn" onPress={goBackToGallery} style={{ marginTop: spacing.lg, alignItems: "center" }}>
  ...
  if (!permission?.granted) {
    return (
      <View style={styles.container} testID="public-camera-permission-screen">
        <GlassHeader title="Camera access" onBack={goBackToGallery} topInset={insets.top} />
  ...
  // ---------- Camera capture step ----------
  return (
    <View style={styles.cameraContainer} testID="public-selfie-camera-screen">
      ...
      <View style={[styles.camHeader, { paddingTop: insets.top + spacing.sm }]}>
        <Pressable testID="camera-back" onPress={goBackToGallery} style={styles.roundBtn} hitSlop={10}>
...
const styles = StyleSheet.create({
  ...
  consentIcon: {
    width: 64, height: 64, borderRadius: radius.pill, backgroundColor: colors.brandTertiary,
